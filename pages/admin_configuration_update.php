...
/* V1.0 : 20130927 */
	...
	$fields = $data->AdminGetTableFields('admin_configuration');
	$values = $data->AdminSpecificGet('admin_configuration', $fields, $id);
	...
	$.ajax({
		url: "_admin_configuration_update.php",
		type: "GET",
		data: data,
		cache: false,
	...
	$script .= '
		...
		var answer = confirm("'.constant('ADMIN_CONFIGURATION_DELETE_QUESTION').'");
        ...
        $.ajax({
            url: "_admin_configuration_delete.php",
            type: "GET",
            data: data,
            cache: false,
            success: function (html) {
...
$script .= '</span><a class="close" href="javascript:void(0)" onclick="$(\'div[id=notification]\').hide();return false;"></a></div>
	<div id="title" xmlns:v="http://rdf.data-vocabulary.org/#">
		<ul class="crumb admin_user_crumb" itemprop="breadcrumb">
			...
			<li typeof="v:Breadcrumb" >
				<span class="arrow">></span><span class="title"><a href="'.$_SESSION[SITE_ID]['admin_configuration_http'].$_SESSION[SITE_ID]['admin_configuration_site_name'].'/pages/admin_configuration_update.php?id='.$id.'" rel="v:url" property="v:title" >'.constant('ADMIN_ADMIN_CONFIGURATION_UPDATE_TITLE').'</A></span> | <a href="'.$_SESSION[SITE_ID]['admin_configuration_http'].$_SESSION[SITE_ID]['admin_configuration_site_name'].'/pages/admin_configuration_add.php" >'.constant('ADMIN_ADMIN_CONFIGURATION_ADD_TITLE').'</A>
			</li>
		</ul>
	</div>
		<div id="main">
		<form method="POST" action="_admin_configuration_update.php">
			<TABLE>
				<TR>
					<TD>
						<TABLE>
	...
	$script .= '	<TR>
					...
					<TR>
						<TD>
						    ...
						    <INPUT TYPE="submit" ID="submit" NAME="submit" VALUE="'.constant('ADMIN_ADMIN_CONFIGURATION_UPDATE_UPDATE').'" ALT="'.constant('ADMIN_ADMIN_CONFIGURATION_UPDATE_UPDATE').'">
							<INPUT TYPE="submit" ID="delete" NAME="delete" VALUE="'.constant('ADMIN_ADMIN_CONFIGURATION_DELETE_DELETE').'" ALT="'.constant('ADMIN_ADMIN_CONFIGURATION_DELETE_DELETE').'">
