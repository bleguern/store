...
/* V1.0 : 20130927 */
	...
	if ($table_name == '') {
		...
		$document_ready_script_data_list .= ' \'specific_table=\' + specific_table.val() + \'&hidden_field=\' + hidden_field.val()';
...
<div id="title" xmlns:v="http://rdf.data-vocabulary.org/#">
	<ul class="crumb admin_specific_crumb" itemprop="breadcrumb">
		...
		<li typeof="v:Breadcrumb" >
			<span class="arrow">></span><span class="title"><a href="'.$_SESSION[SITE_ID]['admin_configuration_http'].$_SESSION[SITE_ID]['admin_configuration_site_name'].'/pages/admin_specific_add.php?'.$parameters.'" rel="v:url" property="v:title" >'.constant('ADMIN_'.strtoupper($table_name).'_ADD_TITLE').'</A></span>
		</li>
	</ul>
</div>
 			<div id="main">
		<form method="POST" action="_admin_specific_add.php">
			<TABLE>
				<TR>
					<TD>
						<TABLE>
	...
	$script .= '			<INPUT TYPE="submit" ID="submit" NAME="submit" VALUE="'.constant('ADMIN_'.strtoupper($table_name).'_ADD_ADD').'" ALT="'.constant('ADMIN_'.strtoupper($table_name).'_ADD_TITLE').'">
